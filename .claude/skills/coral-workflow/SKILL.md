...
**本技能采用连续执行模式：阶段之间自动衔接，无需用户手动介入，直到最终需求目标完成。**
...
2. **直接继续下一阶段** — 不输出任何中断标记，不等待用户输入，直接从下一阶段的第一步开始执行。只需输出简短的阶段过渡信息：
   ```
   ─────────────────────
   阶段 [N] 已完成 → 进入阶段 [N+1]：[阶段名称]
   ─────────────────────
   ```

3. **不中断原则** — 整个工作流从阶段0到阶段9一气呵成，中间无需用户任何手动操作。唯一需要用户介入的时机是：
   - 阶段1需求澄清时的问答
   - 阶段2设计偏好调研和方案选定
   - 阶段3疑问确认
   - 阶段3技术方案选型
   - 重启达到上限时的用户决策
   这些是工作流本身需要用户输入的场景，而非系统层面的中断。

### 工作流重启机制

**当阶段9验证未通过时，自动重启工作流：**

1. **自动重启** — 验证未通过时，不输出中断标记，直接从阶段1重新开始执行：
   - 读取验证报告中的问题列表
   - 在阶段1中重点关注这些问题
   - 保留原始需求文件，不重新创建

2. **重启限制** — 最多允许重启3次，超过后停止自动重启，使用 AskUserQuestion 请求用户决策

3. **重启记录** — 每次重启时更新 `docs/workflow/<task-slug>/restart-history.md`
...
... 连续执行，无需手动干预
  ↓
阶段9验证通过 → [WORKFLOW_COMPLETE] 工作流完成
  ↓
阶段9验证未通过 → 自动重启，从阶段1重新开始（保留原始需求和验证报告）
```
...
4. **直接进入阶段1** — 输出简短过渡信息后，立即开始阶段1的执行：
```
─────────────────────
...
4. **直接进入阶段2** — 输出简短过渡信息后，立即开始阶段2的执行：
```
─────────────────────
阶段 1 已完成：需求分析（产品经理视角）
产出：PRD + 原型
→ 进入阶段 2：UI/UX设计（设计师视角）
─────────────────────
```
...
4. **直接进入阶段3** — 输出简短过渡信息后，立即开始阶段3的执行：
```
─────────────────────
阶段 2 已完成：UI/UX设计（设计师视角）
设计决策：[用户选定的方案]
→ 进入阶段 3：逻辑梳理与技术方案（架构师视角）
─────────────────────
```
...
4. **直接进入阶段4** — 输出简短过渡信息后，立即开始阶段4的执行：
```
─────────────────────
...
4. **直接进入阶段5** — 输出简短过渡信息后，立即开始阶段5的执行：
```
─────────────────────
阶段 4 已完成：测试用例编写（测试视角）
用例统计：功能[X]条、集成[X]条、端到端[X]条
→ 进入阶段 5：任务拆分与分配（项目经理视角）
─────────────────────
```
...
4. **直接进入阶段6** — 输出简短过渡信息后，立即开始阶段6的执行：
```
─────────────────────
阶段 5 已完成：任务拆分与分配（项目经理视角）
任务拆分完成（共[X]个任务）
→ 进入阶段 6：开发执行（交接 coral-frontend）
─────────────────────
```
...
4. **直接进入阶段7** — 输出简短过渡信息后，立即开始阶段7的执行：
```
─────────────────────
阶段 6 已完成：开发执行
...
4. **直接进入阶段8** — 输出简短过渡信息后，立即开始阶段8的执行：
```
─────────────────────
阶段 7 已完成：功能测试（测试工程师视角）
测试结果：通过[X]条、失败[X]条，所有 Bug 已修复
→ 进入阶段 8：回归测试与交付（项目经理视角）
─────────────────────
```
...
4. **直接进入阶段9** — 输出简短过渡信息后，立即开始阶段9的执行：
```
─────────────────────
阶段 8 已完成：回归测试与交付
回归测试全部通过
→ 进入阶段 9：最终验证（产品经理Agent）
─────────────────────
```
...
3. **自动重启** — 不输出中断标记，不等待用户输入，直接从阶段1重新开始：
   ```
   ─────────────────────
   阶段 9 验证未通过
   原因：[验证未通过的具体原因]
   → 自动重启，从阶段 1 重新梳理需求（第 [N] 次重启）
   ─────────────────────
   ```
   然后直接进入阶段1的执行，重点关注验证报告中发现的问题。
...
**核心原则**：每个阶段完成后保存进度并直接进入下一阶段，无需用户手动操作。进度文件同时作为会话中断后的恢复依据。
...
当技能重新触发且检测到 `docs/workflow/<task-slug>/progress.json` 存在时，说明之前的工作流因会话中断而未完成，需要续接：
...
16. **连续执行无中断** — 每个阶段完成后保存进度并直接进入下一阶段，不输出中断标记，不等待用户输入，全程自动衔接直到阶段9验证通过
...
28. **验证不过必重启** — 阶段9验证未通过时，自动从阶段1重新梳理需求，直接继续执行无需用户介入
